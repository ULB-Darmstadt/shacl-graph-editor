...
function classLabelFor(classIri: string, shapes: readonly NodeShape[]): string {
  // Prefer a shape whose targetClass matches.
...
export function buildBrowseModel(store: Store, shapes: readonly NodeShape[]): BrowseModel {
  ...
  // Index: subject IRI → set of class IRIs
  const subjectClasses = new Map<string, Set<string>>()
  // Index: subject IRI → predicate IRI → list of objects
  const subjectProps = new Map<string, Map<string, Array<NamedNode | BlankNode | Literal>>>()
  // Track all subjects (even those without rdf:type)
  ...
  /** Picks the best label for a subject from its property map. */
  ...
  // Pre-resolve labels for every subject so cross-references can be displayed
  // with the referenced subject's label rather than its raw IRI.
  ...
  // Bucket subjects per class. Multi-typed subjects appear in every bucket.
  ...
  for (const subjIri of allSubjects) {
    ...
    if (props) {
      // Sort predicates alphabetically by their resolved label for stable rendering.
      ...
      for (const predicate of sortedPredicates) {
        // Skip the predicate we already used for the label (avoid duplicating it).
