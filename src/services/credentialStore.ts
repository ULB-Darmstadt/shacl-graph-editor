 ...
 * CredentialStore
 *
 * Stores Airtable credentials encrypted at rest in IndexedDB.
 * Uses AES-GCM with a key derived (PBKDF2) from a per-installation
 * random salt + a stable browser identifier. This is NOT cryptographically
 * strong against an attacker with full access to IndexedDB, but it does
 * prevent casual inspection (e.g. via DevTools) and accidental leakage
 * compared to plain localStorage.
 *
 * For production, prefer a session-only flow (PAT entered every time)
 * or proxy the requests through a backend.
 */
...
async function deriveKey(salt: Uint8Array): Promise<CryptoKey> {
  // Derive from a stable string — not a real secret, but better than nothing.
