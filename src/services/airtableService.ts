...
export class AirtableService {
  ...
  /** Lists all tables in the configured base via the Metadata API. */
  ...
  /**
   * Fetches all records from a table, automatically following Airtable's
   * pagination via the `offset` cursor. Respects rate-limits with a
  ...
  async fetchTableRecords(tableIdOrName: string): Promise<AirtableRecord[]> {
    ...
    do {
      ...
      offset = page.offset
      if (offset) await new Promise(r => setTimeout(r, 250)) // gentle throttle
    } while (offset)
    return all
  }

  /**
   * Converts Airtable records into a flat tabular form suitable for the
   * `AirtableDataSource` constructor: shared header set + per-row values.
   */
  static recordsToTable(records: AirtableRecord[]): {
    headers: string[]
    rows: unknown[][]
    recordIds: string[]
  } {
    // Union of all field names across records
