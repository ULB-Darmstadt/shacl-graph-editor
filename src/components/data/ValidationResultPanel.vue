<script setup lang="ts">
import { computed } from 'vue'
import Tag from 'primevue/tag'
import type { ValidationResult, ValidationViolation } from '@/services/shaclValidator'

const props = defineProps<{ result: ValidationResult }>()

const byShape = computed(() => {
  const map = new Map<string, { label: string; violations: ValidationViolation[] }>()
  for (const v of props.result.violations) {
    if (!map.has(v.shapeIri)) map.set(v.shapeIri, { label: v.shapeLabel, violations: [] })
    map.get(v.shapeIri)!.violations.push(v)
  }
  return Array.from(map.entries()).map(([iri, data]) => ({ iri, ...data }))
})

function severityLabel(s: string): 'danger' | 'warn' | 'info' {
  if (s === 'error') return 'danger'
  if (s === 'warning') return 'warn'
  return 'info'
}
</script>

<template>
  <section class="validation-panel">
    <header class="panel-header">
      <h2>Validierungsergebnis</h2>
      <div class="counts">
        <Tag
          v-if="result.errorCount > 0"
          :value="`${result.errorCount} Fehler`"
          severity="danger"
        />
        <Tag
          v-if="result.warningCount > 0"
          :value="`${result.warningCount} Warnungen`"
          severity="warn"
        />
        <Tag
          v-if="result.isValid && result.violations.length === 0"
          value="Alle Constraints erfüllt ✓"
          severity="success"
        />
        <Tag
          v-else-if="result.isValid && result.warningCount > 0"
          value="Gültig (mit Warnungen)"
          severity="warn"
        />
      </div>
    </header>

    <div v-if="result.violations.length === 0" class="empty">
      <i class="pi pi-check-circle" style="color: var(--p-green-500); font-size: 1.2rem" />
      Keine Constraint-Verletzungen gefunden.
    </div>

    <div v-for="group in byShape" :key="group.iri" class="shape-group">
      <h3 class="shape-name">{{ group.label }}</h3>
      <ul class="violation-list">
        <li
          v-for="(v, idx) in group.violations"
          :key="idx"
          class="violation"
          :class="`sev-${v.severity}`"
        >
          <Tag
            :severity="severityLabel(v.severity)"
            :value="v.severity === 'error' ? 'Fehler' : 'Hinweis'"
            style="font-size: 0.7rem; flex-shrink: 0"
          />
          <span class="prop-label">{{ v.propertyLabel }}</span>
          <span class="msg">{{ v.message }}</span>
        </li>
      </ul>
    </div>
  </section>
</template>

<style scoped lang="scss">
.validation-panel {
  border: 1px solid var(--color-border);
  border-radius: var(--radius-md);
  background: var(--color-surface-2);
  overflow: hidden;
}
.panel-header {
  display: flex;
  align-items: center;
  gap: var(--space-3);
  padding: var(--space-3) var(--space-4);
  border-bottom: 1px solid var(--color-border);
  background: var(--color-surface-1);
  h2 { margin: 0; font-size: 1rem; }
}
.counts { display: flex; gap: var(--space-2); flex-wrap: wrap; }
.empty {
  padding: var(--space-4);
  display: flex;
  align-items: center;
  gap: var(--space-2);
  color: var(--p-green-600, #16a34a);
  font-size: 0.9rem;
}
.shape-group {
  border-bottom: 1px solid var(--color-border);
  &:last-child { border-bottom: none; }
}
.shape-name {
  margin: 0;
  padding: var(--space-2) var(--space-4);
  font-size: 0.85rem;
  color: var(--color-accent);
  background: rgba(99, 102, 241, 0.04);
  border-bottom: 1px solid var(--color-border);
}
.violation-list {
  list-style: none;
  padding: 0;
  margin: 0;
}
.violation {
  display: flex;
  align-items: flex-start;
  gap: var(--space-2);
  padding: var(--space-2) var(--space-4);
  border-bottom: 1px solid var(--color-border);
  font-size: 0.83rem;
  &:last-child { border-bottom: none; }
  &.sev-error { background: rgba(239, 68, 68, 0.04); }
  &.sev-warning { background: rgba(245, 158, 11, 0.04); }
}
.prop-label {
  font-family: var(--font-mono);
  font-weight: 600;
  white-space: nowrap;
  color: var(--color-text);
  min-width: 120px;
}
.msg { color: var(--color-text-muted); flex: 1; }
</style>
